import { motion } from 'framer-motion';
import './directlinks-overlay.css';

export default function DirectLinksMotion() {
  return (
    <div className="relative mt-8 flex justify-end w-full" style={{ minHeight: 240 }}>
      <motion.div
        className="absolute top-0 right-0 w-[600px] h-[240px] bg-purple-700 rounded-xl opacity-80 hidden md:block z-10"
        initial={{ opacity: 0, x: 120 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.8, delay: 1.0, type: 'spring', stiffness: 60 }}
      ></motion.div>
      <motion.div
        className="absolute right-[10px] top-[10px] bg-white text-[#1F1B24] rounded-xl p-8 flex flex-col shadow-lg w-full md:w-[600px] h-[240px] md:mr-0 mr-auto z-20 directlinks-overlay"
        initial={{ opacity: 0, x: 100 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.8, delay: 0.7, type: 'spring', stiffness: 60 }}
        whileHover={{ 
          scale: 1.02, 
          boxShadow: '0 8px 32px 0 rgba(80,0,120,0.15)',
          transition: { duration: 0.2 }
        }}
      >
        <h2 className="font-squada mb-2 tracking-wide" style={{ fontSize: '3.125rem' }}>DIRECT LINKS</h2>
        <div className="flex-1"></div>
        <div className="flex flex-row items-end justify-between w-full">
          <motion.a 
            href="https://www.youtube.com/watch?v=dQw4w9WgXcQ"
            target="_blank"
            className="border-2 border-[#1F1B24] text-[#1F1B24] px-[15px] py-[10px] rounded-[5px] text-base font-manrope font-semibold text-center w-fit hover:bg-[#1F1B24] hover:text-white transition-all duration-200" 
            style={{ background: 'none' }}
            whileHover={{ 
              scale: 1.05,
              backgroundColor: '#1F1B24',
              color: '#FFFFFF',
              transition: { duration: 0.2 }
            }}
            whileTap={{ scale: 0.95 }}
          >
            DOWNLOAD RESUME
          </motion.a>
          <div className="flex gap-4">
            <motion.a 
              href="https://github.com/" 
              target="_blank" 
              aria-label="GitHub" 
              className="flex items-center justify-center"
              whileHover={{ 
                scale: 1.2,
                transition: { duration: 0.2 }
              }}
              whileTap={{ scale: 0.9 }}
            >
              <img src="/images/BUTTON-GH.svg" alt="GitHub" className="w-10 h-10" />
            </motion.a>
            <motion.a 
              href="https://linkedin.com/" 
              target="_blank" 
              aria-label="LinkedIn" 
              className="flex items-center justify-center"
              whileHover={{ 
                scale: 1.2,
                transition: { duration: 0.2 }
              }}
              whileTap={{ scale: 0.9 }}
            >
              <img src="/images/BUTTON-LK.svg" alt="LinkedIn" className="w-10 h-10" />
            </motion.a>
          </div>
        </div>
      </motion.div>
    </div>
  );
} 